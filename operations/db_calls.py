...
def show_db(exchange, search, term):
        #Why do we have search as a boolean and then a term option???????????
        if search:
                try:
                    if "::" in term:
                            if exchange == 'all':
                            ...
                            raise raise_error('--search', ':: between object and search term.')
                except:
                    raise raise_error('--term', 'requires a key and value pair with :: between them')
        else:
